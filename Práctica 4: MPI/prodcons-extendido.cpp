...
void buffer(){
   int value[TAM], peticion, pos  =  0, rama, fuente;
   MPI_Status status;
   ...
   for( unsigned int i=0 ; i < ITERS*2 ; i++ ){
      ...
      switch(rama){
         case 0:
            MPI_Recv( &value[pos], 1, MPI_INT, fuente, 1, MPI_COMM_WORLD, &status );
            cout << "Buffer recibe " << value[pos] << " de Productor " << fuente << endl << flush;
            pos++;
            break;
         case 1:
            MPI_Recv( &peticion, 1, MPI_INT, fuente, 0, MPI_COMM_WORLD, &status );
            MPI_Ssend( &value[pos-1], 1, MPI_INT, fuente, 2, MPI_COMM_WORLD);
            cout << "Buffer envía " << value[pos-1] << " a Consumidor " << fuente << endl << flush;
            pos--;
            break;
      }
   }
